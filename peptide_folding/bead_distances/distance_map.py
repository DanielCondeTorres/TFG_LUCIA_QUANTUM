...
from typing import Union, Tuple, DefaultDict, Dict, Optional
import numpy as np
...
class DistanceMap:
    ...
    def displacement_map(self, axis=0) -> DefaultDict[BaseBead, Dict[BaseBead, SparsePauliOp]]:
    ...
    def first_neighbor(
        ...
        Here, we chose, :math:`lambda_0 = 7*(j- i + 1)`.
        ...
        identity_op = _build_full_identity(num_qubits)

        expression = lambda_0 * (x - identity_op) + pair_energies_multiplier * energy * identity_op
    ...
    def second_neighbor(
        ...
        expression = lambda_1 * (2 * identity_op - x) + pair_energies_multiplier * energy * identity_op
        return _fix_qubits(expression)
