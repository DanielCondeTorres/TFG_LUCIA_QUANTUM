...
def _create_contact_qubits(
    ...
    Returns:
        Tuple consisting of dictionaries of SparsePauliOp operators for contacts/interactions between a
    ...
    lower_main_upper_main: Dict[int, Dict[int, SparsePauliOp]] = collections.defaultdict(dict)
    lower_side_upper_main: Dict[int, Dict[int, SparsePauliOp]] = collections.defaultdict(dict)
    lower_main_upper_side: Dict[int, Dict[int, SparsePauliOp]] = collections.defaultdict(dict)
    lower_side_upper_side: Dict[int, Dict[int, SparsePauliOp]] = collections.defaultdict(dict)

    num_contacts = 0
    num_qubits = pow(main_chain_len - 1, 2)
    full_id = _build_full_identity(num_qubits)
    for lower_bead_id in range(1, main_chain_len - 3):  # first bead id is 1
...
def _create_contact_op_for_axis(
    ...
    # We have 4 block positions for combinations of main and side chain beads:
    # (main-main, main-side, side-main, side-side)
    # We'll create a list of operators for each block position.
    blocks = []
    for block_position in range(4):
        if block_position == contact_op_block_position:
            blocks.append(contact_op)
        else:
            blocks.append(_build_full_identity(num_qubits))

    # Tensor the blocks together
    contact_op_padded = blocks[0]
    for block in blocks[1:]:
        contact_op_padded = contact_op_padded.tensor(block)

    return _convert_to_qubits(contact_op_padded)


def _are_beads_in_different_sets(upper_bead_id: int, lower_bead_id: int) -> bool:
    """Determines if beads belong to different sets A and B."""
...
def _are_beads_k_plus_steps_apart(
    ...
    """Checks if beads are at least k steps apart."""
    return (upper_bead_id - lower_bead_id) >= k


def _log_contact(
    lower_bead_id: int, upper_bead_id: int, lower_chain_type: str, upper_chain_type: str
) -> None:
    """Logs possible contacts between beads."""
    logger.info(
...
def _calc_index(chain_len: int, lower_bead_pos: int, upper_bead_pos: int) -> int:
    """Calculates the index for a qubit representing a contact between two beads."""
    # Position variables are indexed from 0
...
def _convert_to_qubits(
    ...
    """Converts a Pauli operator into qubits for the contact Hamiltonian."""
    num_qubits = pauli_op.num_qubits
    full_id = _build_full_identity(num_qubits)
    return (full_id - pauli_op) * 0.5
